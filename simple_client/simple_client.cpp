...
public:
		...
		on_handshake(beast::error_code ec)
	{
		...
		//ws_.async_write(net::buffer("hello"), beast::bind_front_handler(
		//	&session::on_write,
		//	shared_from_this()
		//));
	}
...
int main(int argc, char** argv)
{
	...
	sess->run(host, port, text);
	std::thread websocket_th = std::thread([&ioc] { ioc.run(); });

	while (1) {
		std::string input;
		std::getline(std::cin, input);
		//sess->deffered_send(input);
		sess->send(input);
		//sess->ws().async_write(net::buffer(input), [](beast::error_code, size_t) {});
	}
	websocket_th.join();
	return EXIT_SUCCESS;
}